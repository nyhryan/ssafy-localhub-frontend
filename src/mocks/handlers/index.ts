...
export const handlers = [
  ...
  http.get("/api/v1/categories", async ({ request }) => {
    ...
    const filtered = mockPlaces.filter(
      (place) => !filter || filter === "전체" || place.category_name === filter
    );
  ...
  http.post("/api/v1/chat", async ({ request }) => {
    ...
    const festivalKeyword =
      normalized.includes("축제") ||
      normalized.includes("일정") ||
      normalized.includes("공연");
    const postKeyword =
      normalized.includes("게시글") ||
      normalized.includes("글") ||
      normalized.includes("커뮤니티");

    if (postKeyword) {
      const searchTerm = normalize(
        normalized.replace("게시글", "").replace("커뮤니티", "").trim()
      );
      const matched = mockPosts
        .filter((post) =>
          normalize(`${post.title} ${post.content} ${post.author}`).includes(
            searchTerm
          )
        )
        .slice(0, 3);
    ...
    if (placeKeyword) {
      ...
      const matched = mockPlaces
        .filter((place) =>
          normalize(
            `${place.category_name} ${place.region} ${place.title}`
          ).includes(searchTerm)
        )
        .slice(0, 3);
      return HttpResponse.json({
